...
func (l Lot) ValidatePolicy(maxHazard int, allowed map[string]bool, grace time.Duration, now time.Time) error {
	if l.HazardScore > maxHazard {
		return fmt.Errorf("hazard policy blocks lot")
	}
	if !allowed[l.Chemistry] {
		return fmt.Errorf("chemistry not allowed")
	}
	if l.ExpiresAt != nil && l.ExpiresAt.Add(grace).Before(now) {
		return fmt.Errorf("lot expired")
	}
	return nil
}
