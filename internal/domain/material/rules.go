...
func ValidateLot(candidate Lot) error {
	if candidate.ID == "" || candidate.TenantID == "" {
		return fmt.Errorf("material identity required")
	}
	return ValidateWeight(candidate.Kind, candidate.Grams)
}
