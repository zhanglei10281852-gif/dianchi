...
func ValidateAddition(current Manifest, item Item) error {
	if item.Weight <= 0 || strings.TrimSpace(item.ID) == "" || strings.TrimSpace(item.LotID) == "" {
		return fmt.Errorf("manifest item is invalid")
	}
	for _, old := range current.Items {
		if old.ID == item.ID || old.LotID == item.LotID {
			return fmt.Errorf("manifest item already exists")
		}
	}
	return nil
}
