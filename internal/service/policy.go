...
func (p *PolicyService) Allow(tenant string, lot battery.Lot, now time.Time) error {
	v := p.Get(tenant)
	if lot.HazardScore > v.MaxHazard {
		return apperr.New(apperr.Conflict, "hazard policy blocks lot")
	}
	if !v.AllowedChemistries[lot.Chemistry] {
		return apperr.New(apperr.Invalid, "chemistry not allowed")
	}
	if lot.ExpiresAt != nil && lot.ExpiresAt.Add(v.ExpiryGrace).Before(now) {
		return apperr.New(apperr.Conflict, "lot expired")
	}
	return nil
}
