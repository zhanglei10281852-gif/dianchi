...
func TestRecoveryAuditFailurePreservesEcologicalLedger(t *testing.T) {
	...
	defer f.close()
	lot := createFlow(t, f)
	assertLedger := func(wantState string, recoveries, outbox, idempotency, audits int) {
		t.Helper()
		var state string
		...
		for _, check := range checks {
			...
			default:
				err = f.store.DB.QueryRow(check.query, lot.ID).Scan(&got)
			}
			if err != nil {
