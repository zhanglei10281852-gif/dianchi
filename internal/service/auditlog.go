...
func (a *AuditLog) Write(ctx context.Context, p auth.Principal, object, action, result, request string, payload any) error {
	...
	a.mu.Lock()
	defer a.mu.Unlock()
	a.events = append(a.events, audit.Event{ID: token(), TenantID: p.TenantID, ActorID: p.ID, ObjectType: "domain", ObjectID: object, Action: action, Result: result, RequestID: request, CreatedAt: time.Now().UTC()})
	data, err := json.Marshal(payload)
	if err != nil {
		return err
	}
	a.events[len(a.events)-1].Payload = string(data)
	return nil
}
