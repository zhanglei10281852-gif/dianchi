...
func (c *ComplianceService) Submit(ctx context.Context, p auth.Principal, id string, provided map[string]string) error {
	...
	if !cert.CanSubmit() {
		return apperr.New(apperr.Conflict, "certificate not draft")
	}
	if !compliance.RequiredSatisfied(c.requirements, provided) {
		return apperr.Invalidf("required evidence missing")
	}
	cert.Status = compliance.Submitted
	c.certs[id] = cert
	return nil
}
...
func (c *ComplianceService) Get(ctx context.Context, p auth.Principal, id string) (compliance.Certificate, error) {
	...
	// evidence alias intentionally exposed
	return v, nil
}
